...
# parser.add_argument('--img_size', default=(16, 128, 128), type=int, nargs='+', help='input image size.')
# parser.add_argument('--crop_size', default=(14, 128, 128), type=int, nargs='+', help='cropped image size.')
...
# channel-cutout configs (apply when 'channel_cutout' in transform_list)
...
parser.add_argument('--fixcutchannel', action='store_true', default=False,
                    help='random or fix channel index. If True, cut dwi_b0(c8) & adc(c9)')

parser.add_argument('--num-hier-classes', type=int, default=25, metavar='N',
                    help='number of label classes (Model default if None)')
parser.add_argument('--num-phases', type=int, default=10, metavar='N',
                    help='number of label classes (Model default if None)')
...
def main():
    ...
    model = create_model_custom(
        ...
        ## 是否修改输入序列数量, default: 10
        num_phase=len(args.simpl_phase) if len(args.simpl_phase)>0 else args.num_phases,
    ...
    if args.rank == 0:
        ...
        saver = CheckpointSaver(
                ...
                # TODO
                decreasing=decreasing, max_history=1)
...
def train_one_epoch(
    ...
    for batch_idx, (input, target, sign) in enumerate(loader):
        last_batch = batch_idx == last_idx
        data_time_m.update(time.time() - end)
        # TODO: binary classification
        # target = torch.as_tensor((target == 1) | (target == 2) | (target == 5), dtype=torch.int64)
        # target = torch.as_tensor(target == 5, dtype=torch.int64)
        input, target, sign = input.cuda(), target.cuda(), sign.cuda().float()
        with amp_autocast():
            output = model(input)
