...
class MultiPhaseLiverDataset(torch.utils.data.Dataset):
    def __init__(self, args, is_simpl_phase=False, is_training=True):
        self.args = args
        self.size = args.img_size
        self.is_training = is_training
        if is_simpl_phase:
            self.simpl_phase = args.simpl_phase
        ...
        if is_simpl_phase:
            phase_list = self.simpl_phase
        else:
            phase_list = ['t2', 'dwi(b1)', 'ip', 'op', 'preartery', 'lap', 'pv', 
                    'delay', 'dwi(b0)', 'adc']
        # 平扫: T2、DWI(b0)、DWI(b1)、ADC、IP、OP
        # 增强: T1(Pre)、LAP、PVP、Delay
        if is_training:
        ...
        for item in anno:
            ...
            # 处理需要忽略的标签
            sign_label = np.zeros(shape=(args.num_hier_classes,), dtype=np.int)
            sign_label[[int(x) for x in item[2].split(',') if x != '']] = 1
            sign_list.append(sign_label)
    ...
    def __getitem__(self, index):
        ...
        sign_label = self.sign_list[index]
        return (image, label, sign_label, patient_id)
    ...
    def transforms(self, mp_image, transform_list):
        ...
        if 'channel_cutout' in transform_list:
            mp_image = random_channel_cutout(mp_image, cutout_num=args.cutcnum, p=args.cutcprob, mode=args.cutcmode, fix=args.fixcutchannel)
        if 'random_center_crop' in transform_list:
            mp_image = random_center_crop(mp_image, args.img_size)
