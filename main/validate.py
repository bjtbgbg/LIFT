...
def validate(args):
    ...
    model = create_model(
        args.model,
        num_phase=len(args.simpl_phase) if args.simpl_phase else 10,
    ...
    is_simpl_phase = True if len(args.simpl_phase) > 0 else False
    dataset = MultiPhaseLiverDataset(args, is_simpl_phase=is_simpl_phase, is_training=False)
    ...
    with torch.no_grad():
        ...
        for (input, target, sign) in tqdm(loader):
            target = target.cuda()
            input = input.cuda()
